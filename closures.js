...
Create a function   saveOutput   that accepts
  a function (that will accept one argument),
  and a string (that will act as a password).

saveOutput will then ...
  return a function that behaves exactly like the passed-in function,
  ... except if a password string is passed in as an argument...
    returned function will return ...
      an object with all previously passed-in arguments as keys,
...
function saveOutput(func, magicWord) {
  const loggedOutput = {};
  function checkStore(num) {
    if (num === magicWord) {
      return loggedOutput;
    } else {
      loggedOutput[num] = func(num);
      return loggedOutput[num];
    }
  }
  return checkStore;
}
